...
def encode_registers_numbers(register_name: str):
    if len(register_name) > 2:
        print(f"Error : this is not a register reference expected R<number> format\nline of instruction --> {line_number} : {current_instruction}")
        exit(0)
    if register_name[0] != 'R':
        print(f"Error : invalid register expected R<number> format\nline of instruction --> {line_number} : {current_instruction}")
        exit(0)
    if 7 < int(register_name[1]) or 0 > int(register_name[1]):
        print(f"Error : invalid register range expected  from 0 to 7\nline of instruction --> {line_number} : {current_instruction}")
        exit(0)
...
def encode_assembly_line(instruction: str):
    ...
    instruction_opCode_with_regs = update_opcode_with_registers(instruction_opCode, register_array)
    if not instruction_coded["is_immediate"]:
        return instruction_opCode_with_regs
    instruction_opCode_immediate = immediate_to_binary(instruction_opCode_with_regs,tokenize_instruction[-1],(instruction_coded["is_immediate"] and (instruction_coded["is_ea"]==0)),instruction_coded["is_ea"])
...
def get_decimal(immediate_value:str):
    try:
        ...
        if immediate_int < 0:
            print(f"Error : Negative numbers are not allowed.\nline of instruction --> {line_number} : {current_instruction}")
            exit(0)
...
def immediate_to_binary(instruction:str,immediate_value:str,is_immediate:int,is_ea:int):
        ...
        if is_immediate and  not (0 <= immediate_int <= 0xFFFF ):
                print(f"Error : exceeds 65535 limit.\nline of instruction --> {line_number} : {current_instruction}")
                exit(0)
        if is_ea :
